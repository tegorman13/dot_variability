...
# Simulation Function
simulate <- function(num_categories, num_samples, training_distortion_level, within, c, gamma, nd=6) {
  prototypes <- generate_prototypes(num_categories, num_dimensions=nd, between=2)
...
num_categories <- 3
num_dimensions <- 2
num_samples <- 900 # number of samples per category
between <- 2
within <- 0.210
gamma <- 5.0
c <- 0.475
...
for (distortion_level in distortion_levels) {
  results <- replicate(nsim, simulate(num_categories, num_samples, distortion_level, within, c, gamma, nd=num_dimensions), simplify = FALSE)
...
ggplot(combined_results, aes(x = Pattern_Token, y = correct_classifications, fill = factor(distortion_level))) +
  ...
  theme_minimal() +
  scale_fill_brewer(palette = "Set1") +ggtitle(paste0("nsim: ",nsim,"; gamma: ",gamma,"; c: ",c,"; within: ",within,"; between: ",between,"; num_samples: ",num_samples)) +eg
```
