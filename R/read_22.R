...
pacman::p_load(dplyr,purrr,tidyr,ggplot2, data.table,readr,here, patchwork, conflicted, afex,grateful,rstatix,usedthese)
...
loadPattern="*.txt"
pathString=paste(here::here(pathLoad),"/Data/",sep="")
mFiles <- list.files(path=here::here(pathString),pattern = loadPattern, recursive = FALSE) # should be 89 in exp 2
...
for (i in 1:nFiles){
  ps=paste(here::here(pathString),mFiles[i],sep="")
