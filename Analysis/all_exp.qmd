...
if(html) theme_set(theme_nice())  else theme_set(theme_minimal())
...
all_data |> 
  ...
  axis.text.x = element_text(face = "plain", size = rel(1.1))) + 
...
all_data |> filter(Phase==1) |>
  ...
  facet_wrap(~exp2,scales="free_x") +
   labs(title="Training Performance - All Sbjs.", y="Accuracy",col="Training Condition", x="Trial Bin",
...
## Version of plot above, with additional black line that represents each group average learning curve
all_data |> filter(Phase==1) |>
  group_by(sbjCode, condit,exp2) |> 
  mutate(nTrain=max(trial),
         train_bin = cut(trial,breaks=seq(1,max(trial),length.out=nbins+1),include.lowest=TRUE,labels=FALSE)) |> 
 ...
 # geom_line(data=grp_avg,aes(x=train_bin,y=Corr,group=condit),color="black") +
  ...
  facet_wrap(exp2~condit,scales="free_x") +
   labs(title="Training Performance - individual learning curves.", y="Accuracy",col="Training Condition", x="Trial Bin",
