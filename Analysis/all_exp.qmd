...
pacman::p_load(dplyr,purrr,tidyr,ggplot2,readr,here, patchwork, conflicted)
...
all_data |> filter(Phase==2, Pattern_Token != "special") |>
  ...
  summarise(empirical_stat = mean(Corr), 
            ...
            ci_upper = mean(Corr) + sem) |>
  mutate(method = "standard error") |> 
  ggplot(aes(x=condit, y=empirical_stat, col=exp, group=exp)) +  
  geom_point() +
  geom_line(aes(group = exp)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper)) +
  facet_wrap(~Pattern_Token) + 
    labs(x="Training Condition",y="Accuracy",col="Study") 
...
all_data |> filter(Phase==2, Pattern_Token != "special") |>
  ...
  summarise(empirical_stat = mean(Corr), 
            ...
            ci_upper = mean(Corr) + sem) |>
  mutate(method = "standard error") |> 
  ggplot(aes(x=Pattern_Token, y=empirical_stat, col=exp, group=exp)) +  
  geom_point() +
  geom_line(aes(group = exp)) +
  geom_errorbar(aes(ymin = ci_lower, ymax = ci_upper)) +
  facet_wrap(~condit) + 
  labs(x="Item Type",y="Accuracy",col="Study") 
...
all_data |> filter(Phase==1) |>
  ...
  ggplot(aes(x=Block, y=Corr, color=interaction(exp,condit))) +  
  stat_summary(geom="line",fun=mean)+
...
all_data |> filter(Phase==1) |>
  group_by(sbjCode, condit, Pattern_Token,Block,exp) |>
  summarize(Corr=mean(Corr),.groups="keep") |>
  ggplot(aes(x=Block, y=Corr, color=exp)) +  
  stat_summary(geom="line",fun=mean)+
  stat_summary(geom="errorbar", fun.data=mean_se,width=.1) +
...
all_data |> filter(Phase==1) |>
  group_by(sbjCode, condit,Block,exp) |>
  summarize(Corr=mean(Corr),.groups="keep") |>
  ggplot(aes(x=interaction(condit,Block), y=Corr, col=as.factor(Block))) +  
  stat_summary(geom="line",fun=mean)+
  stat_summary(geom="errorbar", fun.data=mean_se,width=.1) +
  facet_wrap(~exp,scales="free_x") + 
  labs(title="Testing Performance - All Sbjs.", y="Accuracy") +
  theme(legend.position = "top",  ggh4x.axis.nestline  = element_line(colour = "black")) + 
  list(geom_hline(yintercept = c(.33, .66),linetype="dashed", alpha=.5),
   scale_y_continuous(breaks=yt,limits=c(0,1)))
