  ...
  df <- d |> filter(pair_label==Pair) |> slice_head(n=1) 
    ...
    ggplot(aes(x = x, y = y)) +
          ...
          theme(strip.text = element_text(size = 7,hjust=.5),
                panel.spacing.x=unit(-9.0, "lines")) 
}
  ...
  text_transform(
    ...
    fn = function(column) {
      map(column, pat_table_plot) |>
        ggplot_image(height = px(220), aspect_ratio = 2)
    }
  ) |>  
  opt_interactive(page_size_default=5, 
    use_page_size_select= TRUE, use_search=TRUE, use_resizers=TRUE,use_filters=TRUE, page_size_values = c(10, 25, 50, 100))
